...
async def benchmark_httpx(url: str, num_requests: int) -> float:
    ...
    print(f"httpx: {num_requests} requests in {elapsed:.3f}s ({success_count} successful)")
    return elapsed
...
async def benchmark_preq(url: str, num_requests: int) -> float:
    ...
    elapsed = time.time() - start
    print(f"preq:  {num_requests} requests in {elapsed:.3f}s ({success_count} successful)")
    return elapsed
